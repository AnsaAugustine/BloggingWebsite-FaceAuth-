    ...
    if flag_new == 0:
        try:
            ...
            if flag==1:
                ...
                if bio == 'Face-ID':
                    image__ = st.file_uploader("Upload an image", type=["jpg", "jpeg", "png"])
                    if image__ is not None:
                        print("123e23")
                        password1 = password.encode('utf-8')
                        # encrypted_password = bcrypt.hashpw(password1, bcrypt.gensalt()).decode('utf-8')
                        encrypted_password = password
                        photo_url = storage3.child("photos/" + email + ".jpg").put(image__)
                        photo_url = storage3.child("photos/" + email + ".jpg").get_url(None) 
                        user_data = {
                            "email": email,
